...
- **No-hook wait fallback.** The default wait now unblocks on the Stop-hook idle marker OR a settled assistant entry in the transcript jsonl (terminal `stop_reason` plus a `text`/`tool_use` block — the same predicate `on-stop.sh` uses). A teammate whose Stop hook never loaded ends its wait on disk evidence rather than burning the full timeout to a 124. Read-only and offset-anchored; no protocol files are written and a prior turn's settled entry is never mistaken for this turn's completion.
