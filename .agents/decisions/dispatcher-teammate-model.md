...
- **Coordination state lives outside any shared session.** Because the dispatcher and teammates are isolated, the history index, identity records, and scheduling live in `tm` plus the file protocol — not in a context the dispatcher and teammate share.
